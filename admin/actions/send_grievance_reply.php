...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($mail) {
        try {
            ...
            $stmt = $conn->prepare("INSERT INTO grievance_replies (grievance_id, reply_subject, reply_message, sent_at) VALUES (?, ?, ?, NOW())");
            $stmt->execute([$grievance_id, $subject, $message]);
            
            // Update the main grievance table
            $stmt = $conn->prepare("UPDATE student_grievances SET replied = 1, replied_at = NOW() WHERE id = ?");
            $stmt->execute([$grievance_id]);
            
            // Redirect with success message
            header('Location: ../dashboard/view_grievance.php?success=1&message=Reply+sent+successfully');
            exit;
            
        } catch (Exception $e) {
            // Redirect with error message
            header('Location: ../dashboard/view_grievance.php?error=1&message=Failed+to+send+reply:+' . $e->getMessage());
            exit;
        }
    } else {
        // Redirect with error message
        header('Location: ../dashboard/view_grievance.php?error=1&message=Mail+configuration+error');
        exit;
    }
}
?>
